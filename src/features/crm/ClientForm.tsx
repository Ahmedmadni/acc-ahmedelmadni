        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input value={form.phone} onChange={(e) => setField("phone", e.target.value)} placeholder="05xxxxxxxx" className={inputCls} />
            </div>
            <div>
            ...
            <div>
              ...
              <input value={form.national_id} onChange={(e) => setField("national_id", e.target.value)} className={inputCls} />
            </div>
          </div>
        </section>
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.company_name} onChange={(e) => setField("company_name", e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>السجل التجاري</label>
              <input value={form.commercial_register} onChange={(e) => setField("commercial_register", e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>الرقم الضريبي</label>
              <input value={form.tax_number} onChange={(e) => setField("tax_number", e.target.value)} className={inputCls} />
            </div>
            <div>
              ...
              <div className="relative">
                ...
                >
                  ...
                  {Object.entries(BUSINESS_TYPES).map(([val, lbl]) => (
                    <option key={val} value={val} className="bg-[#04101f]">{lbl.ar}</option>
                  ))}
                </select>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className={inputCls + " appearance-none pl-10"}
                >
                  <option value="active" className="bg-[#04101f]">نشط</option>
                  <option value="inactive" className="bg-[#04101f]">غير نشط</option>
                  <option value="pending" className="bg-[#04101f]">معلق</option>
                </select>
                <ChevronDown className="pointer-events-none absolute top-1/2 -translate-y-1/2 left-3 w-4 h-4 text-[#f3d28a]/50" />
              </div>
            </div>
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3 cursor-pointer">
              <input type="checkbox" checked={form.vat_registered} onChange={(e) => setField("vat_registered", e.target.checked)} className="w-4 h-4 accent-[#d7aa52]" />
              <div>
            ...
            <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3 cursor-pointer">
              <input type="checkbox" checked={form.zakat_registered} onChange={(e) => setField("zakat_registered", e.target.checked)} className="w-4 h-4 accent-[#d7aa52]" />
              <div>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(e) => setField("vat_quarter", e.target.value)}
                    className={inputCls + " appearance-none pl-10"}
                  >
                    <option value="quarterly" className="bg-[#04101f]">ربع سنوي</option>
                    <option value="monthly" className="bg-[#04101f]">شهري</option>
                  </select>
                  <ChevronDown className="pointer-events-none absolute top-1/2 -translate-y-1/2 left-3 w-4 h-4 text-[#f3d28a]/50" />
                </div>
              </div>
