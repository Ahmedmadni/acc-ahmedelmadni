...
const SERVICES = [
  ...
  { id: "financial-reports", icon: "📊", ar: "إعداد التقارير المالية والإدارية", en: "Financial & Managerial Reports", descAr: "بناء تقارير دورية تعرض الأداء المالي وتدعم قرار الإدارة التنفيذية.", descEn: "Periodic financial performance reports for executive decision-making.", badge: null, isVat: false },
  { id: "zakat-declaration", icon: "🕌", ar: "إعداد وتقديم الإقرار الزكوي", en: "Zakat Return Preparation", descAr: "حساب وعاء الزكاة وإعداد الإقرار السنوي وفق متطلبات هيئة الزكاة.", descEn: "Calculate zakat base and prepare the annual declaration.", badge: null, isVat: false },
  { id: "bank-reconciliation", icon: "🏦", ar: "التسويات البنكية والضريبية", en: "Bank & Tax Reconciliations", descAr: "تسويات بنكية وإعداد إقرارات ضريبة القيمة المضافة والزكاة بدقة.", descEn: "Bank reconciliations and precise tax declarations.", badge: null, isVat: false },
  { id: "cost-analysis", icon: "📉", ar: "محاسبة التكاليف وتحليل المشاريع", en: "Cost Accounting & Project Analysis", descAr: "تحليل تكاليف المشاريع والمراحل وتقديم رؤية ربحية دقيقة.", descEn: "Project cost analysis and profitability insights.", badge: null, isVat: false },
  { id: "financial-claims", icon: "📋", ar: "إعداد المطالبات المالية", en: "Financial Claims Preparation", descAr: "تنسيق مطالبات احترافية مع الاستشاريين والمقاولين ضمن الحقوق التعاقدية.", descEn: "Professional claims with consultants and contractors.", badge: null, isVat: false },
  { id: "consulting", icon: "💡", ar: "استشارات مالية وإدارية", en: "Financial & Administrative Consulting", descAr: "استشارات للأنشطة التجارية المختلفة بهدف رفع الكفاءة المالية.", descEn: "Business consulting to improve financial efficiency.", badge: null, isVat: false },
  { id: "power-bi", icon: "📈", ar: "لوحات تحليل Excel / Power BI", en: "Excel / Power BI Dashboards", descAr: "تصميم لوحات تفاعلية لمؤشرات الأداء وقراءة بصرية للأرقام.", descEn: "Interactive dashboards for KPIs and visual data reading.", badge: null, isVat: false },
  { id: "website-design", icon: "🌐", ar: "تصميم وتطوير المواقع الإلكترونية", en: "Website Design & Development", descAr: "تصميم مواقع احترافية للأعمال والمكاتب المحاسبية والمتاجر الإلكترونية.", descEn: "Professional websites for businesses and accounting offices.", badge: null, isVat: false },
];
    ...
    <section className="relative py-10">
      ...
      <div className="w-full px-4 sm:px-8 lg:px-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="text-4xl sm:text-5xl font-black leading-tight" style={{ color: "var(--fg)" }}>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {trustBadges.map((b, i) => (
              <div key={i} className="inline-flex items-center gap-1.5 rounded-full border border-[#d7aa52]/25 bg-white/[0.03] px-3 py-1.5 text-[11px] font-semibold text-[#f3d28a]">
                {b.icon}
                {lang === "ar" ? b.ar : b.en}
              </div>
            ))}
        ...
        {submitted ? (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass mt-10 rounded-3xl p-10 text-center">
            ...
            <div className="mt-6 flex justify-center gap-3">
              <button type="button" onClick={() => setSubmitted(false)} className="inline-flex items-center gap-2 rounded-full gold-border px-5 py-2.5 text-xs font-bold" style={{ color: "var(--fg)" }}>
                {lang === "ar" ? "طلب جديد" : "New request"}
              </button>
              <Link to="/" className="inline-flex items-center gap-2 rounded-full bg-gradient-to-br from-[#f3d28a] to-[#b8862e] px-5 py-2.5 text-xs font-bold text-[#04101f]">
                {lang === "ar" ? "الرئيسية" : "Home"}
              </Link>
            </div>
          </motion.div>
        ) : (
          <form id="service-form" onSubmit={onSubmit} className="glass mt-10 space-y-6 rounded-3xl p-6 sm:p-10">
            {/* Service selector */}
            <div>
              <label className="mb-2 block text-sm font-bold" style={{ color: "var(--fg)" }}>
                {lang === "ar" ? "اختر الخدمة المطلوبة" : "Select Required Service"} <span className="text-red-400">*</span>
              </label>
                ...
                >
                  ...
                  {SERVICES.map((s) => (
                    <option key={s.id} value={s.id}>
                      {s.icon} {lang === "ar" ? s.ar : s.en}{s.badge ? " ★" : ""}
                    </option>
                  ))}
                </select>
              ...
              {selectedService && (() => {
                ...
                if (!svc) return null;
                return (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-4 rounded-2xl border border-[#d7aa52]/25 bg-[#d7aa52]/[0.06] p-4 sm:p-5">
                    <div className="flex items-start gap-4">
                      <div className="text-3xl">{svc.icon}</div>
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <div className="text-sm font-bold" style={{ color: "var(--fg)" }}>{lang === "ar" ? svc.ar : svc.en}</div>
                          {svc.badge && (
                        ...
                        {svc.isVat && (
                          <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-bold text-emerald-300">
                            ...
                            {lang === "ar" ? "متوافق مع منصة زاتكا ZATCA" : "Compliant with ZATCA platform"}
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field icon={<User className="w-3.5 h-3.5 text-[#f3d28a]" />} label={lang === "ar" ? "الاسم الكامل" : "Full Name"} required value={name} onChange={setName} placeholder={lang === "ar" ? "أدخل اسمك" : "Enter your name"} />
              <Field icon={<Phone className="w-3.5 h-3.5 text-[#f3d28a]" />} label={lang === "ar" ? "رقم الجوال" : "Phone Number"} required value={phone} onChange={setPhone} type="tel" placeholder="+966 5X XXX XXXX" />
              <Field icon={<Mail className="w-3.5 h-3.5 text-[#f3d28a]" />} label={lang === "ar" ? "البريد الإلكتروني" : "Email Address"} value={email} onChange={setEmail} type="email" placeholder="example@company.com" />
              <Field icon={<Building2 className="w-3.5 h-3.5 text-[#f3d28a]" />} label={lang === "ar" ? "الجهة / الشركة" : "Organization / Company"} value={company} onChange={setCompany} placeholder={lang === "ar" ? "اسم الشركة" : "Company name"} />
              ...
              <SelectField
                ...
                options={[
                  ...
                  { v: "company", l: lang === "ar" ? "شركة (ذ.م.م / مساهمة)" : "Company (LLC / Corp)" },
              ...
              <SelectField
                ...
                options={[
                  ...
                  { v: "urgent", l: lang === "ar" ? "⚡ عاجل — خلال 48 ساعة" : "⚡ Urgent — within 48h" },
            ...
            {/* Details */}
            <div>
              <label className="mb-2 flex items-center gap-1.5 text-sm font-bold" style={{ color: "var(--fg)" }}>
              ...
              <textarea
                ...
                placeholder={lang === "ar" ? "اشرح طلبك بالتفصيل: حجم العمل، المهلة، أي ملاحظات…" : "Describe your request: scope, timeline, any notes…"}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
              <p className="text-[10px] flex items-center gap-1.5" style={{ color: "var(--fg-soft)" }}>
                <Shield className="w-3 h-3 text-[#f3d28a]/60" />
                {lang === "ar" ? "بياناتك محمية ولن تُشارك مع أي طرف ثالث" : "Your data is protected and will not be shared"}
              </p>
              <button type="submit" className="inline-flex items-center gap-2.5 rounded-full bg-gradient-to-br from-[#f3d28a] to-[#b8862e] px-8 py-3.5 text-sm font-black text-[#04101f] shadow-lg shadow-[#d7aa52]/30 hover:scale-105 hover:shadow-[#d7aa52]/50 transition-all active:scale-100">
            ...
            <div className="pt-2 text-center">
              <Link to="/" className="text-xs hover:text-[#d7aa52]" style={{ color: "var(--fg-soft)" }}>
