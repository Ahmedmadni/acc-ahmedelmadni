...
import {
  ...
  updateExamQuestionStatus,
  updateExamQuestion,
  deleteMyExamQuestion,
} from "@/lib/exam-questions.functions";
...
export function ExamPrep({ lang }: { lang: Lang }) {
  ...
  async function runExtract() {
    if (!isAdmin) {
      setUploadMsg({
        ...
        text:
          lang === "ar"
            ? "إدارة بنك الأسئلة متاحة للمديرين فقط."
            : "Question bank management is restricted to admins.",
      });
      return;
    }
    ...
    try {
      ...
      if (valid.length === 0) {
        setUploadMsg({
          kind: "err",
          text: lang === "ar" ? "لم يتم استخراج أي أسئلة" : "No questions extracted",
        });
        return;
      }
      ...
      const payload = valid
        .map((x) => ({
          track: ((TRACKS as readonly string[]).includes(x.track ?? "")
            ? x.track
            : track) as ExamTrack,
          topic: x.topic ?? "—",
          question_ar: x.question_ar ?? x.question_en ?? "",
          question_en: x.question_en ?? "",
          choices_ar: (x.choices_ar?.length === x.choices_en!.length
            ? x.choices_ar
            : x.choices_en) as string[],
          ...
          question_type: ["MCQ", "TRUE_FALSE", "MULTIPLE_RESPONSE", "CASE_STUDY"].includes(
            x.question_type ?? "",
          )
            ? x.question_type
            : "MCQ",
          difficulty: ["easy", "intermediate", "hard"].includes(x.difficulty ?? "")
            ? x.difficulty
            : "intermediate",
      ...
      if (payload.length === 0) {
        setUploadMsg({
          ...
          text:
            lang === "ar"
              ? "كل الأسئلة المستخرجة مكررة."
              : "All extracted questions are duplicates.",
        });
        return;
      }
  ...
  async function migrateLocalToDb() {
    ...
    try {
      ...
      setUploadMsg({
        ...
        text:
          lang === "ar"
            ? `تم ترحيل ${total} سؤال إلى حسابك.`
            : `Migrated ${total} questions to your account.`,
      });
    } catch (e) {
  ...
  function clearLocal() {
    if (!confirm(lang === "ar" ? "حذف جميع الأسئلة المحلية؟" : "Delete all local questions?"))
      return;
    setLocal([]);
    saveLocal([]);
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-[#d7aa52]/20 bg-white/[0.02] px-3 py-2 text-[11px] text-[var(--fg-soft)]">
        ...
        <span>·</span>
        <span>
          {lang === "ar" ? "مدمج:" : "Built-in:"} <b>{SEED_QUESTIONS.length}</b>
        </span>
        {local.length > 0 && (
          <>
            <span>·</span>
            <span>
              {lang === "ar" ? "محلي:" : "Local:"} <b>{local.length}</b>
            </span>
          </>
        )}
        <span>·</span>
        <span>
          {lang === "ar" ? "الإجمالي:" : "Total:"} <b>{allQuestions.length}</b>
        </span>
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-4">
            ...
            <StatTile
              ...
              value={pool.length > 0 ? `${current + 1}/${pool.length}` : "0"}
            />
            <StatTile
            ...
            <StatTile
              ...
              value={score.total ? `${Math.round((score.correct / score.total) * 100)}%` : "—"}
            />
          </div>
          ...
          {!q ? (
            <div className="rounded-xl border border-dashed border-[#d7aa52]/40 p-8 text-center text-sm text-[var(--fg-soft)]">
              {lang === "ar"
                ? "لا توجد أسئلة لهذا المسار. ارفع بنك أسئلة."
                : "No questions for this track yet. Upload a bank."}
            </div>
          ) : (
            <motion.div
              key={q.id}
              ...
              className="rounded-2xl border border-[#d7aa52]/30 bg-gradient-to-br from-white/[0.04] to-transparent p-5"
            >
              ...
              <div className="space-y-2">
                {q.choices[lang].map((c, i) => {
                  ...
                  if (revealed && isCorrect)
                    cls = "border-emerald-400/60 bg-emerald-500/10 text-emerald-100";
                  else if (revealed && isPicked && !isCorrect)
                    cls = "border-red-400/60 bg-red-500/10 text-red-100";
                  return (
                    <button
                      key={i}
                      disabled={revealed}
              ...
              <div className="mt-5 flex items-center justify-between">
                <button
                  onClick={prev}
                  className="inline-flex items-center gap-1 rounded-lg border border-[#d7aa52]/30 bg-white/[0.04] px-3 py-1.5 text-xs font-bold text-[#f3d28a] hover:bg-white/10"
                >
                ...
                <button
                  onClick={next}
                  className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-br from-[#f3d28a] to-[#b8862e] px-4 py-1.5 text-xs font-bold text-[#04101f]"
                >
                  {lang === "ar" ? "التالي" : "Next"}
          ...
          {score.total >= 5 && (
            <div className="flex items-center gap-2 rounded-lg border border-[#d7aa52]/30 bg-[#d7aa52]/5 p-3 text-sm text-[#f3d28a]">
              ...
              {lang === "ar"
                ? `أداؤك: ${score.correct} من ${score.total} (${Math.round((score.correct / score.total) * 100)}%)`
                : `Performance: ${score.correct}/${score.total} (${Math.round((score.correct / score.total) * 100)}%)`}
            </div>
          )}
        </>
      )}
      ...
      {mode === "upload" && (
        <div className="space-y-4 rounded-2xl border border-[#d7aa52]/30 bg-gradient-to-br from-white/[0.04] to-transparent p-5">
          ...
          <textarea
            ...
            placeholder={
              lang === "ar"
                ? "الصق هنا الأسئلة (نص خام أو JSON أو CSV)..."
                : "Paste questions here (raw text, JSON or CSV)..."
            }
          ...
          {uploadMsg && (
            <div
              className={`rounded-lg border p-3 text-sm ${uploadMsg.kind === "ok" ? "border-emerald-400/40 bg-emerald-500/10 text-emerald-200" : "border-red-400/40 bg-red-500/10 text-red-200"}`}
            >
              {uploadMsg.text}
            </div>
          )}
          ...
          {isAdmin && (
            <div className="space-y-3 border-t border-[#d7aa52]/20 pt-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h4 className="text-sm font-extrabold text-[#f3d28a]">
                  {lang === "ar" ? "مراجعة الأسئلة" : "Review questions"}
                </h4>
                <div className="flex gap-2">
                  <button
                    onClick={async () => {
                      await Promise.all(
                        adminQuestions.slice(0, 12).map((item) =>
                          updateStatus({
                            data: { id: item.id, status: "approved", is_public: true },
                          }),
                        ),
                      );
                      await adminQuery.refetch();
                    ...
                    className="rounded-md border border-emerald-400/40 bg-emerald-400/10 px-2 py-1 text-[10px] font-bold text-emerald-200"
                  >
                    {lang === "ar" ? "اعتماد الكل" : "Approve visible"}
                  </button>
                  <button
                    onClick={async () => {
                      await Promise.all(
                        adminQuestions.slice(0, 12).map((item) =>
                          updateStatus({
                            data: { id: item.id, status: "rejected", is_public: false },
                          }),
                        ),
                      );
                      await adminQuery.refetch();
                    ...
                    className="rounded-md border border-amber-400/40 bg-amber-400/10 px-2 py-1 text-[10px] font-bold text-amber-100"
                  >
                    {lang === "ar" ? "رفض الكل" : "Reject visible"}
                  </button>
                </div>
              </div>
              <div className="grid gap-3">
                {adminQuestions.slice(0, 12).map((item) => (
                  <div
                    key={item.id}
                    className="rounded-xl border border-[#d7aa52]/20 bg-[#04101f]/45 p-3"
                  >
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        onClick={async () => {
                          const next = prompt(
                            lang === "ar" ? "تعديل نص السؤال" : "Edit question",
                            item.question[lang],
                          );
                          if (next?.trim()) {
                            await updateQuestion({
                              data: {
                                id: item.id,
                                [lang === "ar" ? "question_ar" : "question_en"]: next.trim(),
                              },
                            });
                            await adminQuery.refetch();
                            await dbQuery.refetch();
                        ...
                        className="rounded-md border border-[#d7aa52]/40 bg-[#d7aa52]/10 px-2 py-1 text-[10px] font-bold text-[#f3d28a]"
                      >
                        {lang === "ar" ? "تعديل" : "Edit"}
                      </button>
                      <button
                      ...
                      <button
                        onClick={async () => {
                          if (confirm(lang === "ar" ? "حذف السؤال؟" : "Delete question?")) {
                            await deleteQuestion({ data: { id: item.id } });
                            await adminQuery.refetch();
                            await dbQuery.refetch();
                          }
                        }}
                        className="rounded-md border border-red-400/40 bg-red-500/10 px-2 py-1 text-[10px] font-bold text-red-200"
                      >
                        {lang === "ar" ? "حذف" : "Delete"}
                      </button>
                    </div>
