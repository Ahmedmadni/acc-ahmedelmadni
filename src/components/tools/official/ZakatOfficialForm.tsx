...
const ADJ_FIELDS: { k: AdjKeys; ar: string; en: string }[] = [
  {
    k: "adj_book_profit_adjustments",
    ar: "التعديلات على صافي الربح",
    en: "Book profit adjustments",
  },
  {
    k: "adj_invalid_maintenance",
    ar: "مصاريف صيانة وإصلاح زائدة عن الحد المسموح",
    en: "Excess maintenance",
  },
  {
    k: "adj_unrecorded_provisions",
    ar: "مخصصات محملة على حسابات الفترة",
    en: "Period-charged provisions",
  },
...
const FP_INTANGIBLES: { k: FpKeys; ar: string; en: string }[] = [
  {
    k: "fp_intangibles",
    ar: "الأصول غير الملموسة (شهرة، براءات)",
    en: "Intangibles (goodwill, patents)",
  },
];
const FP_LIAB_CURRENT: { k: FpKeys; ar: string; en: string }[] = [
  ...
  {
    k: "fp_long_loan_current",
    ar: "قسط مستحق من القروض طويلة الأجل",
    en: "Current portion of LT loans",
  },
...
const FP_LIAB_NONCURRENT: { k: FpKeys; ar: string; en: string }[] = [
  ...
  {
    k: "fp_related_long",
    ar: "مستحق لأطراف ذات علاقة (طويل الأجل)",
    en: "LT related-party payable",
  },
];
...
const ZBASE_ADD: { k: ZBaseAddKeys; ar: string; en: string }[] = [
  ...
  {
    k: "zb_adjusted_profit",
    ar: "صافي ربح/خسارة الزكاة بعد التعديلات",
    en: "Adjusted zakat profit/loss",
  },
  ...
  {
    k: "zb_other_equity",
    ar: "بنود حقوق ملكية أخرى أو مطلوبات أخرى مولت أصول حُسمت من الوعاء",
    en: "Other equity / liabilities funding deducted assets",
  },
...
const ZBASE_DED: { k: ZBaseDedKeys; ar: string; en: string }[] = [
  ...
  {
    k: "zd_invest_outside",
    ar: "الاستثمارات في منشآت خارج المملكة",
    en: "Investments outside KSA",
  },
  {
    k: "zd_invest_zakat",
    ar: "الاستثمارات في منشآت داخل المملكة تخضع للزكاة",
    en: "Investments in zakat-paying KSA entities",
  },
  { k: "zd_carried_losses", ar: "خسائر مرحلة معدلة", en: "Adjusted carried-forward losses" },
  {
    k: "zd_real_estate_dev",
    ar: "عقارات ومشاريع تحت التطوير معدة للبيع",
    en: "Real-estate development for sale",
  },
...
const TX_DED: { k: TxDedKeys; ar: string; en: string }[] = [
  {
    k: "tx_ded_capital_gain",
    ar: "مكاسب رأسمالية محققة من الأوراق المالية المعفاة نظاماً",
    en: "Exempt capital gains",
  },
  {
    k: "tx_ded_dividends_listed",
    ar: "توزيعات نقدية / عينية من شركات مدرجة",
    en: "Dividends from listed companies",
  },
  {
    k: "tx_ded_dividends_invest",
    ar: "توزيعات من استثمارات في شركات معفاة نظاماً",
    en: "Dividends from exempt investments",
  },
  {
    k: "tx_ded_long_loss",
    ar: "خسائر معدّلة بعد أقصى 6% من الربح المعدل طبقاً للائحة",
    en: "Carried losses (capped at 6%)",
  },
  {
    k: "tx_ded_profit_share",
    ar: "حصة من أرباح شركة مستثمر فيها",
    en: "Share of investee profits",
  },
];
...
const SUMMARY_FIELDS: { k: SumKeys; ar: string; en: string }[] = [
  ...
  {
    k: "sm_contracts_disclosed",
    ar: "قيمة الدفعات للعقود المفرح عنها",
    en: "Payments for disclosed contracts",
  },
...
export function ZakatOfficialForm({ lang }: { lang: Lang }) {
  ...
  const totals = useMemo(() => {
    const totalIncomeOp = ["rev_contracts", "rev_insurance", "rev_operating"].reduce(
      (a, k) => a + eff(k),
      0,
    );
    ...
    const cogs =
      eff("cogs_opening") + eff("cogs_imports") + eff("cogs_local") - eff("cogs_closing");
    ...
    const taxableShare =
      (adjustedProfit + totalTxAdd - totalTxDed) * (entity.nonSaudiOwnership / 100);
    const incomeTaxDue = Math.max(0, taxableShare) * TAX_RATE;

    // Summary
    const totalPrepaid = eff("sm_prepaid_1") + eff("sm_prepaid_2") + eff("sm_prepaid_3");
    const totalFines =
      eff("sm_late_filing_fine") + eff("sm_late_payment_fine") + eff("sm_installments_fine");
  ...
  const inputsForArchive: Record<string, number> = {
    ...v,
    _saudi: entity.saudiOwnership,
    _nonSaudi: entity.nonSaudiOwnership,
  };
  ...
  const a = useDeclarationActions({
    ...
    resultFields: Object.keys(resultsForArchive).map((k) => ({
      key: k,
      ar: k,
      en: k,
      helpAr: "",
  ...
  return (
    <div>
      ...
      <SectionCard title={t("(أ) الدخل", "(A) Income")}>
        <p className="mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("الإيرادات من النشاط التشغيلي", "Operating revenue")}
        </p>
        ...
        <TotalRow
          label={t("إجمالي الإيرادات من النشاط التشغيلي", "Total operating revenue")}
          value={totals.totalIncomeOp}
        />

        <p className="mt-4 mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("الإيرادات الأخرى", "Other income")}
        </p>
        {INCOME_FIELDS.slice(3).map((f) => (
          <MoneyRow key={f.k} label={t(f.ar, f.en)} value={get(f.k)} onChange={set(f.k)} />
        ))}
        <TotalRow
          label={t("إجمالي الإيرادات الأخرى", "Total other income")}
          value={totals.totalIncomeOther}
        />
        <TotalRow
          label={t("إجمالي الإيرادات", "Total income")}
      ...
      <SectionCard title={t("(ب) التكاليف والمصاريف", "(B) Costs & Expenses")}>
        <p className="mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("تكلفة البضاعة المباعة", "Cost of goods sold")}
        </p>
        {COGS_FIELDS.map((f) => (
          <MoneyRow key={f.k} label={t(f.ar, f.en)} value={get(f.k)} onChange={set(f.k)} />
        ))}
        ...
        <TotalRow
          label={t("إجمالي المصاريف وتكلفة البضاعة", "Total expenses + COGS")}
          value={totals.totalExpensesWithCogs}
        />
        <TotalRow
          label={t("صافي الربح / (الخسارة) الدفترية", "Book profit / (loss)")}
          value={totals.bookProfit}
          highlight
        />
      </SectionCard>
      ...
      <SectionCard
        title={t(
          "(ج) التعديلات على صافي الربح/الخسارة الدفترية",
          "(C) Adjustments to book profit/(loss)",
        )}
      >
        {ADJ_FIELDS.map((f) => (
          <MoneyRow
        ...
        <TotalRow
          label={t("إجمالي التعديلات", "Total adjustments")}
          value={totals.totalAdjustments}
        />
        <TotalRow
          label={t("صافي الربح / الخسارة المعدّل", "Adjusted profit / loss")}
          value={totals.adjustedProfit}
          highlight
        />
      </SectionCard>
      ...
      <SectionCard title={t("(د) المركز المالي", "(D) Financial Position")}>
        ...
        <TotalRow
          ...
          value={totals.totalCurAssets}
        />

        <p className="mt-4 mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("أصول غير متداولة", "Non-current assets")}
        </p>
        ...
        <TotalRow
          ...
          value={totals.totalNonCurAssets}
        />

        <p className="mt-4 mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("أصول غير ملموسة", "Intangibles")}
        </p>
        ...
        <p className="mt-4 mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("خصوم متداولة", "Current liabilities")}
        </p>
        ...
        <TotalRow
          ...
          value={totals.totalCurLiab}
        />

        <p className="mt-4 mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("خصوم غير متداولة", "Non-current liabilities")}
        </p>
        ...
        <TotalRow
          ...
          value={totals.totalNonCurLiab}
        />

        <p className="mt-4 mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("حقوق الشركاء", "Partners' equity")}
        </p>
        {FP_EQUITY.map((f) => (
          <MoneyRow key={f.k} label={t(f.ar, f.en)} value={get(f.k)} onChange={set(f.k)} />
        ))}
        ...
        <TotalRow
          label={t("إجمالي الخصوم وحقوق الشركاء", "Total liabilities + equity")}
          value={totals.totalLiabEquity}
          highlight
        />
      </SectionCard>
      ...
      <SectionCard title={t("(هـ) الوعاء الزكوي", "(E) Zakat Base")}>
        ...
        <TotalRow
          label={t(
            `الزكاة المستحقة على الشريك السعودي (${entity.saudiOwnership}%)`,
            `Zakat on Saudi share (${entity.saudiOwnership}%)`,
          )}
      ...
      <SectionCard
        title={t(
          "(و) الوعاء الضريبي (لحصة الشركاء غير السعوديين)",
          "(F) Tax Base (Non-Saudi share)",
        )}
      >
        <p className="mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("الإضافات الضريبية", "Tax additions")}
        </p>
        {TX_ADD.map((f) => (
          <MoneyRow
            key={f.k}
        ...
        <p className="mt-4 mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("الحسميات الضريبية", "Tax deductions")}
        </p>
        {TX_DED.map((f) => (
          <MoneyRow
            key={f.k}
        ...
        <TotalRow
          label={t(
            `الدخل الخاضع للضريبة على حصة غير السعودي (${entity.nonSaudiOwnership}%)`,
            `Taxable income (Non-Saudi ${entity.nonSaudiOwnership}%)`,
          )}
          value={totals.taxableShare}
        />
        <TotalRow
          label={t("ضريبة الدخل المستحقة (20%)", "Income tax due (20%)")}
          value={totals.incomeTaxDue}
          highlight
        />
      </SectionCard>
      ...
      <SectionCard title={t("(ز) الملخص", "(G) Summary")}>
        <p className="mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("الفرق الضريبي", "Tax difference")}
        </p>
        <MoneyRow
          label={t("إجمالي ضريبة الدخل المستحقة", "Total income tax due")}
          value={totals.incomeTaxDue}
          onChange={() => {}}
        />
        ...
        <TotalRow
          label={t(
            "فروقات الضريبة المطلوب دفعها أو المسددة بالزيادة",
            "Tax difference payable / overpaid",
          )}
          value={totals.taxDifference}
        />
        ...
        <p className="mt-4 mb-2 text-[11px] font-bold text-[#f3d28a]/80">
          {t("الفرق الزكوي", "Zakat difference")}
        </p>
        <TotalRow label={t("صافي الزكاة المستحقة", "Net zakat due")} value={totals.netZakat} />
        <TotalRow
          label={t("الإجمالي الزكوي المستحق (مع الغرامات)", "Total zakat owed (incl. fines)")}
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <div className="rounded-md border border-emerald-400/40 bg-emerald-400/10 p-3">
            <div className="text-[11px] font-bold text-emerald-200">
              {t("سداد - رقم الفاتورة للزكاة", "SADAD invoice (Zakat)")}
            </div>
            <div dir="ltr" className="mt-1 font-mono text-base font-extrabold text-emerald-100">
              {sadadZakat}
            </div>
          </div>
          <div className="rounded-md border border-sky-400/40 bg-sky-400/10 p-3">
            <div className="text-[11px] font-bold text-sky-200">
              {t("سداد - رقم الفاتورة للضريبة", "SADAD invoice (Tax)")}
            </div>
            <div dir="ltr" className="mt-1 font-mono text-base font-extrabold text-sky-100">
              {sadadTax}
            </div>
          </div>
        </div>
        ...
        <div className="mt-3">
          <ActionsBar
            ...
            refLinks={[
              ...
              {
                ar: "دليل الإقرار الزكوي والضريبي",
                en: "Zakat & Tax Filing Guide",
